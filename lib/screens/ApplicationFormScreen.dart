...
class _ApplicationFormScreenState extends State<ApplicationFormScreen> {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildTextField(
                            ...
                            readOnly: true, // Email is from Firebase Auth
                            validator: (value) {
                              if (value == null || value.isEmpty) {
  ...
  Future<Map<String, String>> _uploadFiles() async {
    ...
    if (_profilePicFile != null) {
      final fileName =
          'profile_pics/$userId/$timestamp${path.extension(_profilePicFile!.path)}';
      final ref = _storage.ref().child(fileName);

      final uploadTask = ref.putFile(_profilePicFile!);
      final snapshot = await uploadTask.whenComplete(() {});
      final downloadUrl = await snapshot.ref.getDownloadURL();

      urls['profilePicUrl'] = downloadUrl;
    } else if (_profilePicUrl != null) {
      urls['profilePicUrl'] = _profilePicUrl!;
    }

    // Upload resume if selected
    if (_resumeFile != null) {
      final fileName =
          'resumes/$userId/$timestamp${path.extension(_resumeFile!.path)}';
      final ref = _storage.ref().child(fileName);

      final uploadTask = ref.putFile(_resumeFile!);
      final snapshot = await uploadTask.whenComplete(() {});
      final downloadUrl = await snapshot.ref.getDownloadURL();

      urls['resumeUrl'] = downloadUrl;
    } else if (_resumeUrl != null) {
      urls['resumeUrl'] = _resumeUrl!;
    }
  ...
  Future<void> _submitApplication() async {
    ...
    try {
      ...
      final Map<String, String> fileUrls = await _uploadFiles();
      ...
      final applicationData = {
        ...
        // URLs
        'resumeUrl': fileUrls['resumeUrl'] ?? _resumeUrl ?? '',
        'profilePicUrl': fileUrls['profilePicUrl'] ?? _profilePicUrl ?? '',
      };
